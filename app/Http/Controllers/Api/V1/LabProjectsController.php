...
class LabProjectsController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        return ApiResponse::make(
            data: $labProjects->map(fn (LabProject $labProject): array => (new LabProjectResource($labProject))->resolve($request))->values(),
            meta: $this->collectionFallbackMeta($request, $labProjects),
            links: ['self' => url('/api/v1/labs')],
        );
    }

    /**
     * @param  Collection<int, LabProject>  $labProjects
     ...
     */
    private function collectionFallbackMeta(Request $request, Collection $labProjects): array
    {
        $localeMeta = PublicApiLocale::resolve($request);
        $missingFields = [];
        $fallbackFields = [];

        foreach ($labProjects->values() as $index => $labProject) {
            $labProjectMeta = (new LabProjectResource($labProject))->fallbackMeta($request);
            $missingFields = array_merge($missingFields, $this->prefixFields("labs.$index", $labProjectMeta['missingFields']));
            $fallbackFields = array_merge($fallbackFields, $this->prefixFields("labs.$index", $labProjectMeta['fallbackFields']));
        }
